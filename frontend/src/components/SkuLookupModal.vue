          ...
          <div class="divide-y divide-stone-100">
            <div v-for="it in g.items" :key="it.code" class="px-3 py-2.5" :class="it.avail > 0 ? 'bg-emerald-50/30' : ''">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <!-- the POOL's number: what the Orders board can actually pick -->
                <span v-if="it.pickable !== undefined" class="inline-flex items-center gap-1 text-[10px] font-bold rounded px-1.5 py-0.5 ring-1"
              ...
              <div v-if="it.bins.length" class="flex flex-wrap gap-1 mt-1.5">
                <span v-for="b in it.bins" :key="b.bin" class="inline-flex items-center gap-1 text-[10.5px] font-mono rounded px-1.5 py-0.5"
                  ...
                  <Icon name="map-pin" :size="9" />{{ b.bin.replace(' - JM', '') }} · {{ b.net }}<template v-if="b.veto"> · {{ t('sku.reserveTag') }}</template>
                </span>
              </div>
            </div>
          </div>

<template>
  <div class="max-w-[980px] mx-auto px-4 py-6 space-y-4">
    <header class="flex items-center justify-between gap-3 flex-wrap">
      ...
      <div v-if="po && filledCount" class="flex items-center gap-2">
        <span class="inline-flex items-center gap-1.5 text-[12px] font-semibold text-emerald-700 bg-emerald-50 ring-1 ring-emerald-200 rounded-lg px-2.5 h-8 tabular-nums">
          {{ filledCount }} {{ t('consol.items') }} · {{ totalUnits }} {{ t('recv.units') }}
        </span>
    ...
    <!-- Scanner (optional): a piece, or a PO number -->
    ...
    <!-- PO open: header + target + the PO's items (review / enter, scanner optional) -->
    <template v-else>
        ...
        <div class="flex items-start gap-3 flex-wrap">
          ...
          <div class="flex-1 min-w-[180px]">
            <input v-model="target" list="lp-gi-targets"
                   ...
                   style="--tw-ring-color: var(--accent-400)" />
            <datalist id="lp-gi-targets"><option v-for="w in boot?.warehouses || []" :key="w" :value="w" /></datalist>
          </div>
      ...
      <!-- The PO's items: enter received qty per line (type or scan) -->
      <div class="bg-white rounded-2xl ring-1 ring-stone-200/70 shadow-sm overflow-hidden">
        <div class="px-4 py-2.5 border-b border-stone-100 flex items-center gap-2 flex-wrap">
          <span class="text-[12px] font-semibold text-stone-900">{{ t('gi.poItems') }}</span>
          <span class="text-[11px] text-stone-400 tabular-nums">{{ poLines.length }} {{ t('slotting.skus') }}</span>
          <div class="relative ms-auto">
            <span class="absolute inset-y-0 left-2.5 flex items-center text-stone-400"><Icon name="search" :size="13" /></span>
            <input v-model="lineSearch" :placeholder="t('gi.filterPh')"
                   class="h-8 w-[170px] max-w-full ps-8 pe-2 rounded-lg ring-1 ring-stone-200 bg-white text-[12px] outline-none focus:ring-2 focus:ring-[var(--accent-400)]" />
          </div>
          <button class="h-8 px-2.5 rounded-lg text-[11.5px] font-semibold text-emerald-700 bg-emerald-50 ring-1 ring-emerald-200 hover:bg-emerald-100" @click="fillAll">{{ t('gi.fillAll') }}</button>
          <button class="h-8 px-2.5 rounded-lg text-[11.5px] font-semibold text-stone-600 bg-stone-100 hover:bg-stone-200" @click="clearAll">{{ t('gi.clearAll') }}</button>
        </div>

        <div class="divide-y divide-stone-50 max-h-[440px] overflow-y-auto">
          <div v-for="l in shownLines" :key="l.itemCode" class="px-4 py-2.5 flex items-center gap-3"
               :class="l.qty > 0 ? (l.qty > l.pending ? 'bg-amber-50/60' : 'bg-emerald-50/40') : ''">
            ...
            <span class="text-[10.5px] font-semibold rounded px-1.5 py-0.5 tabular-nums whitespace-nowrap flex-shrink-0"
              ...
              <template v-if="l.extra">{{ t('gi.notOnPo') }}</template>
              <template v-else>{{ l.received }}/{{ l.ordered }} · {{ l.pending }} {{ t('gi.leftShort') }}</template>
            </span>
            <div class="flex items-center gap-1 flex-shrink-0">
              <button class="w-8 h-8 rounded-lg bg-stone-100 text-[14px] font-bold" @click="l.qty = Math.max(0, (l.qty || 0) - 1)">−</button>
              <input v-model.number="l.qty" type="number" min="0" inputmode="numeric"
                     class="w-14 h-8 text-center text-[14px] font-bold tabular-nums rounded-lg ring-1 ring-stone-200 focus:outline-none focus:ring-2 focus:ring-[var(--accent-400)]" />
              <button class="w-8 h-8 rounded-lg bg-stone-100 text-[14px] font-bold" @click="l.qty = (l.qty || 0) + 1">+</button>
            </div>
          </div>
          <p v-if="!shownLines.length" class="px-4 py-8 text-center text-[12.5px] text-stone-400">{{ t('gi.noLineMatch') }}</p>
        </div>
    ...
    <div class="bg-white rounded-xl ring-1 ring-stone-200/70 overflow-hidden">
      <div class="px-4 py-2.5 border-b border-stone-100"><span class="text-[12px] font-semibold text-stone-900">{{ t('gi.recentTitle') }}</span></div>
...
const poLines = ref([]);       // [{ itemCode, sku, name, image, ordered, received, pending, qty, extra? }]
...
const filledCount = computed(() => poLines.value.filter((l) => (l.qty || 0) > 0).length);
const totalUnits = computed(() => poLines.value.reduce((s, l) => s + (l.qty || 0), 0));
...
const shownLines = computed(() => {
  ...
  if (!q) return poLines.value;
  return poLines.value.filter((l) =>
    (l.sku || "").toLowerCase().includes(q) || (l.name || "").toLowerCase().includes(q) || l.itemCode.includes(q));
});
...
async function selectPo(name, bump = null) {
  try {
    ...
    poSearch.value = "";
    lineSearch.value = "";
    if (bump) bumpLine(bump.itemCode, bump);
...
function closePo() { po.value = null; poLines.value = []; note.value = ""; lineSearch.value = ""; setTimeout(() => scanner.value?.refocus(), 50); }
...
// Add one unit to a line; if the item isn't on the PO, append an over-receipt line.
function bumpLine(itemCode, meta) {
  const line = poLines.value.find((l) => l.itemCode === itemCode);
  if (line) { line.qty = (line.qty || 0) + 1; return line; }
  const extra = { itemCode, sku: meta?.sku || "", name: meta?.name || itemCode,
                  image: meta?.image || "", ordered: 0, received: 0, pending: 0, qty: 1, extra: true };
  poLines.value.push(extra);
  return extra;
}

function fillAll() { poLines.value.forEach((l) => { if (!l.extra) l.qty = l.pending; }); }
function clearAll() { poLines.value = poLines.value.filter((l) => !l.extra).map((l) => ({ ...l, qty: 0 })); }
...
async function scanToOpen(code) {
  ...
  if (res.ok) {
    ...
    if (res.pos.length === 1) { scanner.value?.showSuccess(`${res.pos[0].po} · ${res.name}`); return selectPo(res.pos[0].po, res); }
    chooser.value = res;
...
async function scanIntoPo(code) {
  ...
  const line = bumpLine(res.itemCode, res);
  if (res.onPo && !line.extra) scanner.value?.showSuccess(`${res.name} · ×${line.qty}`);

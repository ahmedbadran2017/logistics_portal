    ...
    <div v-if="!loadError" class="bg-white rounded-xl ring-1 ring-stone-200/70 overflow-hidden">
        ...
        <table class="w-full min-w-[760px]">
          ...
          <tbody class="divide-y divide-stone-100">
            <tr v-for="p in shown" :key="p.no" class="transition-colors cursor-pointer hover:bg-stone-50"
              ...
              <td class="px-4 py-2.5"><PlLife v-if="p.life" :life="p.life" compact /><div v-else-if="p.pct > 0" class="flex items-center gap-2"><div class="flex-1 h-1.5 rounded-full bg-stone-100 overflow-hidden"><div class="h-full rounded-full" :class="p.errors ? 'bg-rose-500' : 'bg-emerald-500'" :style="{ width: p.pct + '%' }" /></div><span class="text-[10.5px] text-stone-400 tabular-nums w-[28px]">{{ p.pct }}%</span></div><span v-else class="text-[11px] text-stone-300">—</span></td>
      ...
      <div v-if="isLiveData && total > pageSize" class="flex items-center justify-between px-4 py-2.5 border-t border-stone-100 bg-stone-50/50">
        ...
        <div class="flex items-center gap-1">
          <button :title="t('common.prev')" class="pager-btn" :disabled="page <= 1" @click="page--; load(true)"><Icon name="chevron-left" :size="13" class="flip-rtl" /></button>
          ...
          <button :title="t('common.next')" class="pager-btn" :disabled="page * pageSize >= total" @click="page++; load(true)">
                <Icon name="chevron-right" :size="13" class="flip-rtl" /></button>
        </div>
      </div>
    </div>

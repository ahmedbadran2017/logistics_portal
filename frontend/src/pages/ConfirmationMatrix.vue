<template>
  <!-- Agent × city, for the confirmation lane.
       It used to live on the manager's Team matrix beside the warehouse
       floor, under one heading, on a different window (12 rolling weeks
       against the floor's 30 days). Two teams whose work has nothing in
       common do not belong in one table, and the people who actually read
       this — the lane's own section admins — could not reach the manager's
       page at all. Ahmed, 2026-09-21. -->
  <div class="max-w-[1200px] mx-auto px-4 py-6 space-y-4">
    <header class="flex items-end justify-between gap-3 flex-wrap">
      <div>
        <h1 class="text-[20px] font-bold text-stone-900 tracking-tight">{{ t('cfmx.title') }}</h1>
        <p class="text-[12.5px] text-stone-500 mt-0.5 max-w-[660px]">{{ t('cfmx.intro') }}</p>
      </div>
      <div class="flex items-center gap-3 flex-wrap">
        ...
        <div class="flex items-center gap-1.5">
          <button v-for="b in ['deliver', 'confirm', 'overall']" :key="b"
                  ...
                  @click="basis = b; load()">{{ t('cmx.b_' + b) }}</button>
        </div>
      </div>
    </header>
    ...
    <template v-else-if="cc && cc.rows.length">
      <section class="flex items-center gap-3 flex-wrap">
        <div v-for="k in headline" :key="k.label" class="rounded-xl bg-white ring-1 ring-stone-200/70 px-4 py-2.5">
        ...
        <p class="text-[11px] text-stone-400 max-w-[400px] leading-relaxed">{{ t('cfmx.ccNote') }}</p>
      </section>
      ...
      <section class="bg-white rounded-xl ring-1 ring-stone-200/70 overflow-x-auto">
        <div class="p-3 min-w-[860px]">
          ...
          <div v-for="r in cc.rows" :key="r.user" class="grid items-center gap-1 mb-1" :style="cols">
...
async function load() {
  loading.value = true;
  try {
    cc.value = await api("contact_center.agent_matrix", {
      basis: basis.value, days: days.value,
      frm: frm.value || undefined, to: to.value || undefined,
    });
  } catch { cc.value = null; }
  loading.value = false;
}

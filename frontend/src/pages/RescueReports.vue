<template>
  <div class="p-5 sm:p-6 space-y-4 max-w-[1100px] mx-auto">
    <header class="flex items-start justify-between gap-3 flex-wrap">
      <div>
        <h1 class="text-[20px] font-bold text-stone-900 tracking-tight">{{ t('rsr.title') }}</h1>
        <p class="text-[12.5px] text-stone-500 mt-0.5">{{ t('rsr.intro') }}</p>
      </div>
    ...
    <template v-else-if="data">
          ...
          <table class="w-full min-w-[760px] text-[12.5px]">
            <thead>
              <tr class="text-[10.5px] font-semibold uppercase tracking-[0.05em] text-stone-400 border-b border-stone-100">
                ...
                <th class="text-end px-3 py-2.5">{{ t('rs.actRedeliver') }}</th>
                <th class="text-end px-3 py-2.5">{{ t('rs.thReship') }}</th>
                <th class="text-end px-3 py-2.5">{{ t('rs.thReturn') }}</th>
                <th class="text-end px-3 py-2.5">{{ t('cf.actDna') }}</th>
                <th class="text-end px-3 py-2.5">{{ t('cfr.thTotal') }}</th>
                <th class="text-end px-4 py-2.5">{{ t('rsr.thRescueRate') }}</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-stone-100">
              <tr v-for="a in data.agents" :key="a.user" class="hover:bg-stone-50">
                ...
                <td class="px-3 py-2.5 text-end tabular-nums font-semibold text-emerald-600">{{ a.redeliver }}</td>
                <td class="px-3 py-2.5 text-end tabular-nums text-violet-600">{{ a.reship }}</td>
                <td class="px-3 py-2.5 text-end tabular-nums text-rose-600">{{ a.returnreq + a.cancel }}</td>
                  ...
                  <div class="flex items-center justify-end gap-2">
                    <div class="w-[64px] h-1.5 rounded-full bg-stone-100 overflow-hidden">
                      <div class="h-full rounded-full"
                           :class="a.rescueRate >= 50 ? 'bg-emerald-500' : a.rescueRate >= 30 ? 'bg-amber-500' : 'bg-rose-500'"
                           :style="{ width: a.rescueRate + '%' }" />
                    </div>
                    <span class="tabular-nums font-semibold w-[44px]"
                          :class="a.rescueRate >= 50 ? 'text-emerald-600' : a.rescueRate >= 30 ? 'text-amber-600' : 'text-rose-600'">
                      {{ a.rescueRate }}%
                    </span>
                  </div>
                </td>
      ...
      <div class="grid md:grid-cols-2 gap-3">
        <div class="bg-white rounded-xl ring-1 ring-stone-200/70 overflow-hidden">
          <div class="px-4 py-2.5 border-b border-stone-100 text-[12px] font-semibold text-stone-900">{{ t('rsr.reasonsTitle') }}</div>
          ...
          <div class="divide-y divide-stone-50">
            <div v-for="f in [...data.funnel].reverse()" :key="f.date" class="px-4 py-2 space-y-1">
              <div class="flex items-center gap-3 text-[12px] tabular-nums">
                ...
                <span class="inline-flex items-center gap-1.5 text-emerald-600 font-semibold"><span class="w-2 h-2 rounded-full bg-emerald-500" />{{ f.saved }}</span>
                <span class="inline-flex items-center gap-1.5 text-rose-600"><span class="w-2 h-2 rounded-full bg-rose-400" />{{ f.lost }}</span>
              ...
              <div class="flex h-1.5 rounded-full overflow-hidden bg-stone-100 ms-[76px]">
                <div class="bg-emerald-500" :style="{ width: barW(f.saved) }" />
                <div class="bg-rose-400" :style="{ width: barW(f.lost) }" />

...
_DEFAULTS = {
    ...
    "positiveWords": ["راضي", "ممتاز", "رائع", "شكرا", "satisfait", "merci", "oui", "👍", "نعم"],
    "negativeWords": ["غير راضي", "مشكلة", "مشكل", "سيء", "متأخر", "ناقص", "مكسور", "pas satisfait", "problème", "probleme", "non", "👎", "لا"],
...
def save_settings(payload=None):
    ...
    for k in _DEFAULTS:
        if isinstance(payload, dict) and k in payload:
            cur[k] = payload[k]
    cur["enabled"] = bool(cur.get("enabled")) and bool(cur.get("template"))
...
def _asked_today():
    return frappe.db.sql(
        f"SELECT COUNT(*) FROM `tab{DT}` WHERE asked_at >= %s",
        (now_datetime().replace(hour=0, minute=0, second=0, microsecond=0),))[0][0]
...
def run_ask():
    ...
    try:
        ...
        for r in rows:
            ...
            if frappe.db.sql(f"SELECT 1 FROM `tab{DT}` WHERE phone = %s AND asked_at >= %s LIMIT 1",
                             (phone, cooldown)):
            ...
            try:
                ...
                _record(r, phone, "failed", error=str(e)[:300])
            frappe.db.commit()
    except Exception:
...
def _verdict(text, cfg):
    ...
    if re.fullmatch(r"[1-5]", t):
        return "positive" if int(t) >= 4 else "negative"
    ...
    try:
        ...
        open_rows = frappe.db.sql(
            f"""SELECT name, sales_order, phone, city, customer_name, asked_at, wa_message_id
                FROM `tab{DT}` WHERE status = 'asked' AND asked_at >= %s""",
            (window,), as_dict=True)
        if open_rows:
            ...
            for m in msgs:
                ...
                if not row:
                    # A typed answer with no context: the newest open question
                    # for that phone, and only once it was asked.
                    cands = [r for r in by_phone.get(_digits(m["from"]), [])
                             if r.asked_at <= m.creation]
                    row = cands[-1] if cands else None
                if not row or row.name in done:
                    continue
                v = _verdict(m.message, cfg)
                if not v and m.content_type == "text":
...
def _route(row, verdict, msg, cfg):
    ...
    try:
        if verdict == "positive":
            if cfg.get("thanksText"):
                _send_text(phone, cfg["thanksText"], row.sales_order)
        else:
            if cfg.get("sorryText"):
                _send_text(phone, cfg["sorryText"], row.sales_order)
            ticket = _open_ticket(row, msg, cfg)
    except Exception as e:
        frappe.db.set_value(DT, row.name, "error", str(e)[:300], update_modified=False)
    if ticket:
...
def board(days=14):
    ...
    out["counts"] = counts
    out["sent"] = sent
    out["responseRate"] = round(100.0 * answered / sent, 1) if sent else None
    ...
    out["negatives"] = frappe.db.sql(
        f"""SELECT name, sales_order, customer_name, city, phone, reply, replied_at, ticket,
            ...
            FROM `tab{DT}` WHERE status = 'negative' AND asked_at >= %s
...
def test_send(phone):
    ...
    name, wamid = _send_template(cfg, p, frappe.session.user, None)
    frappe.db.commit()
